...
# チケット生成
@app.route("/ticket/pdf/<int:bulk_id>")
@login_required
def ticket_pdf(bulk_id):
    ...
    SELECT
        ...
        sr.seatNumber,
        bb.bulkBookingId
    FROM t_bulkBooking bb
    JOIN t_seatReservationStatus srs ON bb.bulkBookingId = srs.bulkBookingId
    JOIN t_seatReservation sr ON srs.seatReservationId = sr.seatReservationId
    JOIN t_scheduledShowing ss ON sr.scheduledShowingId = ss.scheduledShowingId
    JOIN t_movies m ON ss.moviesId = m.moviesId
    JOIN t_screen sc ON ss.screenId = sc.screenId
    WHERE bb.bulkBookingId = %s
    """

    cur.execute(sql, (bulk_id,))
    ...
    response.headers['Content-Disposition'] = f'inline; filename=ticket_{bulk_id}.pdf'

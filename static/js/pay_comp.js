...
document.addEventListener('DOMContentLoaded', function() {
    ...
    try {
        ...
        if (completeButtons.length > 0) {
            ...
            console.log(`${completeButtons.length}個の完了ボタンにイベントを設定しました`);
        } else {
            console.warn('complete-button要素が見つかりません');
        }
        ...
        console.error('DOMContentLoaded初期化でエラー:', error);
    }
});
...
function handleComplete(event) {
    try {
        ...
        console.error('完了ボタン処理でエラー:', error);
    }
}
...
function showConvenienceInstructions(result) {
    try {
        ...
        if (!container) {
            console.warn('container要素が見つかりません');
            return;
        }
        ...
        console.error('コンビニ案内表示でエラー:', error);
    }
}
...
function showPayPayInstructions(result) {
    try {
        ...
        console.error('PayPay案内表示でエラー:', error);
    }
}
...
function animatePaymentDetails() {
    try {
        ...
        console.error('アニメーション処理でエラー:', error);
    }
}
